...
use crate::commands::SyncProjectsPayload;
...
async fn handle_request(
    ...
    match req.type_.as_str() {
        ...
        // Web client project push: mirror the browser's project list into the
        // in-memory registry + broadcast `projects_changed` to all clients.
        // In standalone termul-server mode (no desktop), the web client IS
        // the host — this is the only write path for cross-browser persistence.
        "remote_sync_projects" => {
            let parsed: SyncProjectsPayload = match serde_json::from_value(req.payload.clone()) {
                Ok(p) => p,
                Err(e) => {
                    warn!(
                        target: "termul::web::ws",
                        error = %e,
                        "remote_sync_projects: malformed payload"
                    );
                    return WsReply::err(
                        id,
                        WsErrorCode::Unsupported,
                        format!("malformed remote_sync_projects payload: {e}"),
                    );
                }
            };
            // Update in-memory registry + broadcast to all connected clients.
            registry.set(parsed.projects.clone(), parsed.default_project_id.clone());
            broadcast_projects_changed(relay, parsed.default_project_id.as_deref());

            // VPS disk persistence (with rollback). In standalone termul-server
            // mode this is the ONLY write path — without it projects vanish on
            // server restart. Desktop-hosted mode skips this (no file registry).
            if let (Some(file_registry), Some(path)) = (registry_persistence.as_ref(), projects_file) {
                let mut file_registry = file_registry.lock();
                let old_roots = file_registry.roots().to_vec();
                let old_default = file_registry.default_project_id().map(str::to_string);
                let new_roots: Vec<crate::acp::VfsRoot> = parsed.projects.iter().map(|p| {
                    crate::acp::VfsRoot {
                        id: p.id.clone(),
                        name: p.name.clone(),
                        path: std::path::PathBuf::from(&p.path.clone().unwrap_or_default()),
                        color: p.color.clone(),
                        is_archived: p.is_archived,
                        mcp_servers: Vec::new(),
                    }
                }).collect();
                let new_registry = crate::acp::FileProjectRegistry::from_roots(new_roots, parsed.default_project_id.clone());
                match new_registry.save_atomic(path) {
                    Ok(()) => {
                        // Update the file registry in-place to match disk.
                        *file_registry = new_registry;
                    }
                    Err(e) => {
                        warn!(target: "termul::web::ws", error = %e, "remote_sync_projects: disk persistence failed");
                        // Rollback: re-save the old state
                        let restored = crate::acp::FileProjectRegistry::from_roots(old_roots, old_default);
                        let _ = restored.save_atomic(path);
                        *file_registry = restored;
                    }
                }
            }
