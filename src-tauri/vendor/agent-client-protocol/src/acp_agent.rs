    ...
    > {
        match &self.server {
            crate::schema::McpServer::Stdio(stdio) => {
                ...
                // Termul patch: on Windows, suppress the console window that
                // would otherwise flash when launching a GUI-spawned agent
                // subprocess. CREATE_NO_WINDOW = 0x08000000. Mirrors the same
                // flag Termul sets on all its other background subprocesses.
