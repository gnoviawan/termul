...
function paradigmChalkrail() {
  ...
  for (let pi = R; pi < LINES.length; pi += R) {
    const prevEnd = Math.max(...LINES.slice(pi - R, pi).map((L) => L.words[L.words.length - 1].end))
...
function setpiecePapermat() {
  ...
  const scatT = +Math.min(
    Math.max(
      I + (N - 1) * STEP + 0.5,
      Math.min(theme.hero.exitAt ?? LASTWORD.start + 0.02, DUR - 0.48, DUR - 0.32 - (N - 1) * 0.045)
    ),
    DUR - 0.48
  ).toFixed(3)
  const VIG = p.vig ?? 0.3
