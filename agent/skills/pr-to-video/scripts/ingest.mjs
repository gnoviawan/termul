...
const commitLines = commits
  .map(
    (c) => c?.messageHeadline || (c?.messageBody || '').split('\n')[0] || (c?.oid || '').slice(0, 7)
  )
  .filter(Boolean)
...
lines.push(`${baseRef} ← ${headRef} · +${additions} / -${deletions} across ${changedFiles} file(s)`)
...
if (commitLines.length) {
  ...
  if (commitLines.length > MAX_COMMITS) lines.push(`- …(+${commitLines.length - MAX_COMMITS} more)`)
  lines.push('')
}
