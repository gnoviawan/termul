...
import { GitMerge, AlertTriangle, FileCode, ArrowUpCircle, X, Loader2 } from 'lucide-react'
				...
				<div className="w-[500px] max-w-[90vw] bg-popover border border-border rounded-lg shadow-2xl flex flex-col">
						...
						<div className="px-4 py-3 space-y-3">
							...
							{preview.conflictFiles.length > 0 && (
								<div className="space-y-1 max-h-[180px] overflow-auto">
									<p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Conflicted Files</p>
									{preview.conflictFiles.map((file) => (
										<div key={file.path} className="flex items-center gap-2 px-2 py-1 rounded text-xs bg-destructive/5">
											...
											<span className={cn(
												...
												file.severity === 'high' ? 'bg-destructive/10 text-destructive' :
												file.severity === 'medium' ? 'bg-yellow-500/10 text-yellow-500' :
												'bg-green-500/10 text-green-500'
											)}>
												{file.severity}
											</span>
							...
							{preview.changedFiles.length > 0 && (
								<div className="space-y-1 max-h-[100px] overflow-auto">
									<p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Files that will change</p>
									{preview.changedFiles.map((file) => (
										<div key={file} className="flex items-center gap-2 px-2 py-0.5 text-xs text-muted-foreground">
							...
							{preview.conflictFiles.length === 0 && preview.changedFiles.length === 0 && (
								<p className="text-xs text-muted-foreground text-center py-4">No conflicts detected. Ready to merge.</p>
							)}
					...
					{!preview && !loading && !error && (
						<div className="text-center py-12">
							...
							<p className="text-sm text-muted-foreground">Select a merge direction to preview conflicts.</p>
						</div>
					)}
				</div>
			</div>
