...
import {
  FolderTree, Wifi, WifiOff, RefreshCw, FileEdit, Pencil,
  Loader2, ChevronRight, ChevronDown, Folder, File, Link2, Trash2, FolderPlus, FilePlus,
} from 'lucide-react'
...
export function SSHFileExplorer({
  connectionId, isConnected, sftpReady, entries, currentPath,
  expandedDirs, childEntries, loadingDirs, isLoadingRoot, profileName,
  onConnect, onBrowseFiles, onToggleDir, onLoadDir,
  onMkdir, onCreateFile, onDelete, onRename,
  ...
  const handleDelete = useCallback((entry: SFTPEntry) => {
    onDelete(entry)
  }, [onDelete])

  const handleRename = useCallback((entry: SFTPEntry) => {
    onRename(entry)
  }, [onRename])

  const formatSize = (b: number): string =>
    b < 1024 ? `${b} B` : b < 1048576 ? `${(b / 1024).toFixed(1)} KB` : b < 1073741824 ? `${(b / 1048576).toFixed(1)} MB` : `${(b / 1073741824).toFixed(1)} GB`
  ...
  const renderEntry = (entry: SFTPEntry, depth = 0) => {
    ...
    return (
      <div key={entry.path}>
        <div className="flex items-center gap-1 px-2 py-0.5 hover:bg-accent/50 cursor-pointer group text-xs"
          style={{ paddingLeft: `${depth * 14 + 8}px` }}
          onClick={() => isDir ? onToggleDir(entry.path) : handleOpenFile(entry)}>
          {isDir && <span className="flex-shrink-0 w-3.5">{isLoading ? <Loader2 className="h-3 w-3 animate-spin text-muted-foreground" /> : isExp ? <ChevronDown className="h-3 w-3 text-muted-foreground" /> : <ChevronRight className="h-3 w-3 text-muted-foreground" />}</span>}
          {!isDir && <span className="w-3.5" />}
          <Icon className={cn('h-3.5 w-3.5 flex-shrink-0', isDir ? 'text-blue-400' : 'text-muted-foreground')} />
          <span className="flex-1 truncate">{entry.name}</span>
          {!isDir && <span className="text-[10px] text-muted-foreground hidden group-hover:inline">{formatSize(entry.size)}</span>}
    ...
    <div className="w-full h-full flex flex-col">
      <div className="h-9 flex items-center justify-between px-3 border-b border-border">
        ...
        <div className="flex items-center gap-0.5">
          {isConnected ? (
            <>
              <button onClick={onMkdir} className="p-1 rounded hover:bg-accent text-muted-foreground" title="New folder"><FolderPlus className="h-3 w-3" /></button>
              <button onClick={onCreateFile} className="p-1 rounded hover:bg-accent text-muted-foreground" title="New file"><FilePlus className="h-3 w-3" /></button>
              <button onClick={() => onLoadDir(currentPath)} className="p-1 rounded hover:bg-accent text-muted-foreground" title="Refresh"><RefreshCw className="h-3 w-3" /></button>
            </>
          ) : (
            <button onClick={onConnect} className="p-1 rounded hover:bg-accent text-green-500" title="Connect"><Wifi className="h-3.5 w-3.5" /></button>
          )}
        </div>
      </div>
      ...
      {isConnected && (
        <div className="px-3 py-1 border-b border-border">
          <span className="text-[10px] text-muted-foreground font-mono truncate block">{currentPath}</span>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto">
        {!isConnected ? (
          <div className="flex flex-col items-center justify-center h-full px-4 text-center gap-2">
            ...
            <button onClick={onConnect} className="px-3 py-1 text-xs rounded bg-primary text-primary-foreground hover:bg-primary/90">Connect</button>
          </div>
        ) : !sftpReady ? (
          <div className="flex flex-col items-center justify-center h-full px-4 text-center gap-2">
          ...
          <div className="p-4 text-center"><p className="text-xs text-muted-foreground">Empty directory</p></div>
        ) : (
