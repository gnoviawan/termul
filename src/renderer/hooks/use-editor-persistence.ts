...
function createEditorPersistenceSignature(editorState: {
	openFiles: Map<string, EditorFileState>;
	activeFilePath: string | null;
}): string {
	const openFiles = Array.from(editorState.openFiles.entries())
...
function filterExpandedDirsByRoot(
	expandedDirs: string[],
	rootPath?: string,
): string[] {
	if (!rootPath) {
...
function serializePaneTree(node: PaneNode): PersistedPaneNode {
	if (node.type === "leaf") {
		const tabs: PersistedTabRef[] = node.tabs.flatMap(
			(tab): PersistedTabRef[] => {
				if (tab.type === "editor") {
...
function sanitizePaneNode(node: PaneNode): PaneNode | null {
	...
	const survivingEntries = node.children
		...
		.filter(
			(entry): entry is { child: PaneNode; originalIndex: number } =>
				entry.child !== null,
		);
...
function createTerminalMatcher(
	...
	const liveTerminalsById = new Map(
		liveTerminals.map((terminal) => [terminal.id, terminal]),
	);
	...
	const consumeLiveTerminal = (terminalId: string): string | null => {
		...
		const index = unusedLiveTerminals.findIndex(
...
function reconcileTerminalTabs(
	...
	const { hasLiveTerminals, matchTerminalId } = createTerminalMatcher(
		liveTerminals,
		layout,
	);
	const shouldKeepPersistedTerminalTabs =
		!hasLiveTerminals && !!layout?.terminals.length;
...
function deserializePaneTree(persisted: PersistedPaneNodeInput): PaneNode {
	if (persisted.type === "leaf") {
		const tabs: WorkspaceTab[] = (
			"tabs" in persisted ? persisted.tabs : []
...
export function useEditorPersistence(projectId: string): void {
	...
	useEffect(() => {
		...
		async function restore(): Promise<void> {
			...
			try {
				...
				const filteredExpandedDirs = filterExpandedDirsByRoot(
				...
				for (const file of persisted.openFiles) {
					...
					try {
						...
						if (file.isDirty && file.draftContent) {
							...
							const currentState = freshEditorState.openFiles.get(
								file.filePath,
							);
							if (currentState) {
				...
				if (persisted.paneLayout) {
					...
					const openFilePaths = new Set(
						useEditorStore.getState().openFiles.keys(),
					);
					const liveProjectTerminals = useTerminalStore
						.getState()
						.terminals.filter(
							(terminal) =>
								terminal.projectId === projectId && !!terminal.ptyId,
						);
					const persistedTerminalLayout =
						await loadPersistedTerminals(projectId);
					if (isStale()) {
						return;
					}
					...
					useWorkspaceStore
						.getState()
						.loadProjectWorkspace(cleanTree, persisted.activePaneId);
				} else {
					...
					useWorkspaceStore
						.getState()
						.syncEditorTabs(openFilePaths, persisted.activeTabId);
				}
	...
	useEffect(() => {
		...
		let lastEditorSignature = createEditorPersistenceSignature(
		...
		const unsubEditor = useEditorStore.subscribe((state) => {
			const nextSignature = createEditorPersistenceSignature(state);
			if (nextSignature === lastEditorSignature) {
				return;
			}

			lastEditorSignature = nextSignature;
			schedulePersist();
		});
