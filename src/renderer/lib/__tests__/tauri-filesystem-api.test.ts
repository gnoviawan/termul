...
vi.mock("@tauri-apps/plugin-fs", () => ({
	...
	watchImmediate: vi.fn(async (_paths: string[], _callback: WatchCallback) =>
		vi.fn(),
	),
}));
...
describe("tauriFilesystemApi", () => {
	...
	describe("readFile", () => {
		it("should successfully read file content", async () => {
			...
			vi.mocked(stat).mockResolvedValue(
				makeFileInfo({ size: 13, mtime: testDate }),
			);
	...
	describe("writeFile", () => {
		it("should successfully write file", async () => {
			const result = await tauriFilesystemApi.writeFile(
				"/test/file.txt",
				"content",
			);
	...
	describe("createFile", () => {
		it("should successfully create new file", async () => {
			const result = await tauriFilesystemApi.createFile(
				"/test/new.txt",
				"content",
			);
	...
	describe("renameFile", () => {
		it("should successfully rename file", async () => {
			const result = await tauriFilesystemApi.renameFile(
				"/test/old.txt",
				"/test/new.txt",
			);
