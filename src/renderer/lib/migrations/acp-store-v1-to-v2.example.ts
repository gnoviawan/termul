...
export async function migrateAcpStoreV1toV2(): Promise<IpcResult<void>> {
  try {
    ...
    const rawData = await store.get<{
      _version: number
      data: Record<string, AcpSessionV1 | AcpSessionV2>
    }>('acp-sessions')
    ...
    // Check if already migrated to v2 (idempotency)
    if (rawData._version === 2) {
      // Already migrated, no-op
      return { success: true, data: undefined }
    }

    // Detect if data is already v2-shaped (check first session)
    const firstSession = Object.values(rawData.data)[0]
    if (firstSession && Array.isArray(firstSession.messages) && firstSession.messages.length > 0) {
      const firstMessage = firstSession.messages[0]
      // V2 messages have 'id', 'role', 'blocks' fields
      if ('id' in firstMessage && 'role' in firstMessage && 'blocks' in firstMessage) {
        // Data is already v2 format, just update version marker
    ...
    const sessionsV1 = rawData.data as Record<string, AcpSessionV1>
