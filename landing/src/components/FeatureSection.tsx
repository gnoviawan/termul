  ...
  return (
    <section id="features" className="py-32 px-6 max-w-7xl mx-auto relative">
      <div className="flex flex-col lg:flex-row gap-16 lg:gap-24 relative items-start">
        ...
        {/* Left Sticky Sidebar */}
        <div className="lg:w-1/3 lg:sticky lg:top-32 flex flex-col gap-12">
          <div>
          ...
          <div className="hidden lg:flex flex-col gap-1 relative">
            <div 
              className="absolute left-0 right-0 bg-white/10 rounded-lg transition-transform duration-300 pointer-events-none"
              style={{
                height: '44px',
                transform: `translateY(${features.findIndex(f => f.id === activeFeature) * 48}px)`,
            ...
            {features.map((feature) => (
              <a 
                ...
                onClick={(e) => {
                  e.preventDefault();
                  const target = document.getElementById(`feature-${feature.id}`);
                  if (target) {
